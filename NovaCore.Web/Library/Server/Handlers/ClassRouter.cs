...
public class ClassRouter : IHttpRequestHandler
{
    ...
    private static void LoadRoute(IHttpRequestHandler root)
    {
    ...
    public async Task Handle(IHttpContext context, Func<Task> next)
    {
        ...
        foreach (string parameter in context.Request.RequestParameters)
        {
            ...
            else
            {
                ...
                handler = await returnedTask.ContextIndependent();
            }
        ...
        await handler.Handle(context, next).ContextIndependent();
    }

    private static Func<IHttpContext, IHttpRequestHandler, string, Task<IHttpRequestHandler>> GetIndexerRouter(Type arg)
    {
        return GetIndexer(arg) is { } indexer ? CreateIndexerFunction<IHttpRequestHandler>(arg, indexer) : null;
    }
    ...
    private static MethodInfo GetIndexer(Type arg)
    {
        ...
        {
            return Attribute.IsDefined(m, typeof(IndexerAttribute))
                   && m.GetParameters().Length == 2
                   && typeof(Task<IHttpRequestHandler>).IsAssignableFrom(m.ReturnType);
        }

        return arg.GetMethods().SingleOrDefault(Match);
    }
    ...
    private static Func<IHttpRequestHandler, IHttpRequestHandler> CreateRoute(Tuple<Type, string> arg)
    {
        ...
        if (item1.GetProperty(item2) is not { } propertyInfo)
            return null;
